...
export function ensureCurriculumState(state) {
  ...
  if (!state.curriculum) {
    state.curriculum = {
      ...
      phases: (state.phases || []).map((phaseItem) => ({
        id: phaseItem.id || uid(),
        name: phaseItem.name,
        assignedMemberIds: Array.isArray(phaseItem.assignedMemberIds) ? phaseItem.assignedMemberIds : [],
  ...
  for (const phaseItem of state.curriculum.phases || []) {
    if (!Array.isArray(phaseItem.assignedMemberIds)) phaseItem.assignedMemberIds = [];
  }
...
function enrichTopic(topicItem) {
  return {
    ...
    activities: topicItem.activities || [],
    assignedMemberIds: Array.isArray(topicItem.assignedMemberIds) ? topicItem.assignedMemberIds : [],
...
function ensureCurriculumTopics(curriculum) {
  ...
  for (const phaseItem of curriculum.phases) {
    if (!Array.isArray(phaseItem.assignedMemberIds)) phaseItem.assignedMemberIds = [];
    const nestedTopics = (phaseItem.projects || []).flatMap((project) => project.topics || []);
    phaseItem.topics = [...(phaseItem.topics || []), ...nestedTopics].map((topicItem) => enrichTopic(topicItem));
    delete phaseItem.projects;
  }
}
