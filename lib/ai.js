...
export async function discoverResourcesWithAI(topic, curriculum, context = {}) {
  return openAIJson({
    instructions: [
      'You are a learning resource researcher.',
      'Use web search to find resources for the current topic using the supplied project context.',
      'Return exactly three high-quality resources unless no suitable resources exist.',
      'Prioritize official documentation, cppreference, man7, RFCs, Beej\'s Guide, MDN, reputable technical blogs, high-quality educational videos, and official project documentation.',
      'Avoid low-quality blogs, AI-generated spam websites, and duplicate resources.',
      'Choose resources based on educational value, not popularity alone.'
    ].join('\n'),
    input: [{
      role: 'user',
      content: [{
        ...
        text: JSON.stringify({
          ...
          suggestedSearchQuery: context.searchContext || `${curriculum && curriculum.title ? `${curriculum.title} ` : ''}${topic.name}`,
...
function resourceSchema() {
  return {
    type: 'object',
    additionalProperties: false,
    required: ['title', 'source', 'description', 'url', 'type', 'reason'],
    properties: {
      title: { type: 'string' },
      source: { type: 'string' },
